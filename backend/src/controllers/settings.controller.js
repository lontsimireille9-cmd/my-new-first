...
export async function getSettings(req, res, next) {
  try {
    ...
    return res.json({
      theme: { ...DEFAULT_THEME, ...(snap.data()?.settings?.theme || {}) },
      language: snap.data()?.settings?.language === "en" ? "en" : DEFAULT_LANGUAGE,
    });
  } catch (error) {
    next(error);
  }
...
export async function updateSettings(req, res, next) {
  try {
    ...
    const language = req.body?.language === "en" ? "en" : (current.data()?.settings?.language === "en" ? "en" : DEFAULT_LANGUAGE);
    ...
    await db.collection("users").doc(req.user.uid).set({ settings: { theme, language }, updatedAt: new Date().toISOString() }, { merge: true });
    return res.json({ theme, language });
  } catch (error) {
    next(error);
  }
}
